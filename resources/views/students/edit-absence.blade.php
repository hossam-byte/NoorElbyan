...
<x-layout>
    ...
    <x-slot:heading>
        تعديل الغياب اليومي لطلاب المؤسسة
    </x-slot:heading>
                    ...
                    <div class="tab-pane fade show active" id="first-duration" role="tabpanel">
                        <div class="card">
                            ...
                            <div class="card-body d-flex align-items-center gap-2 mb-3">
                                <form id="fetchRecordsForm" action="{{route('students.absence.edit')}}" method="GET" class="d-flex gap-2">
                                    @csrf
                                    <!-- Class Name Dropdown -->
                                    <select id="columnFilter" class="form-select" name="absence-filter-className">
                            ...
                            @if($selectedMorningAbsence)
                                <form id="first-duration-absence-update" action="{{route('students.absence.store')}}" method="POST" class="table-responsive text-nowrap">
                                    @csrf
                                    ...
                                    <table class="table table-striped" id="dataTable-1">
                                        ...
                                        @foreach($selectedMorningAbsence as $absenceRecord)
                                            <input type="hidden" name="academy_class_id" value="{{$absenceRecord->academyClass->id}}">
                                            ...
                                            <tr>
                                                <td>
                                                    ...
                                                    <span class="fw-medium ms-1">{{$absenceRecord->student->name}}</span>
                                                    <input type="hidden" name="students[{{$absenceRecord->student->id}}][student_id]" value="{{$absenceRecord->student->id}}">
                                                </td>
                                                <td>
                                                    <input type="hidden" name="students[{{$absenceRecord->student->id}}][is_present]" value="false">
                                                    <div class="form-check d-flex justify-content-center">
                                                        <input name="students[{{$absenceRecord->student->id}}][is_present]" class="form-check-input student-checkbox" type="checkbox" value="true" {{$absenceRecord->is_present? 'checked': ''}} />
                                                    </div>
                                                </td>
                                            </tr>
                                        @endforeach
                            ...
                            @if($selectedAfternoonAbsence)
                                <form id="first-duration-absence-update" action="{{route('students.absence.store')}}" method="POST" class="table-responsive text-nowrap">
                                    ...
                                    <table class="table table-striped" id="dataTable-1">
                                        ...
                                        <tbody class="table-border-bottom-0">
                                        @foreach($selectedAfternoonAbsence as $absenceRecord)
                                            <input type="hidden" name="academy_class_id" value="{{$absenceRecord->academyClass->id}}">
                                            <input type="hidden" name="date" value="{{$absenceRecord->date}}">
                                            <tr>
                                                <td>
                                        ...
                                        <tr>
                                            <th>عدد الحضور</th>
                                            <th>
                                                <span class="afternoon-selected-count present">0</span>
                                            </th>
                                        </tr>
                                        <tr>
                                            <th>عدد الغياب</th>
                                            <th>
                                                <span class="afternoon-selected-count absent">0</span>
                                            </th>
                                        </tr>
                                        </tfoot>
                                    </table>
