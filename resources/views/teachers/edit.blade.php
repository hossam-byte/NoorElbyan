<x-layout>
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="row">
            <div class="col-12">
                <form method="POST" action="{{ route('teachers.update', $teacher->id) }}" class="card" enctype="multipart/form-data">
                    @csrf
                    @method('PATCH')
                    <div
                            ...
                            <div class="col-lg-8 mx-auto">
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" id="name" name="name" class="form-control" value="{{$teacher->name}}" placeholder="احمد جميل" required />
                                        <x-form-err name="name"/>
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label" for="gender">نوع المعلم</label>
                                        <select id="gender" name="gender" class="form-select">
                                            <option {{$teacher->gender === 'انثي'? 'selected': ''}} value="انثي">انثي</option>
                                            <option {{$teacher->gender === 'ذكر'? 'selected': ''}} value="ذكر">ذكر</option>
                                        </select>
                                        <x-form-err name="gender"/>
                                    </div>
                                    <div class="col-md-6">
                                        <label for="birthdate" class="form-label">تاريخ الميلاد</label>
                                        <input type="date" id="birthdate" name="birthdate" value="{{$teacher->birthdate}}" class="form-control" required />
                                        <x-form-err name="birthdate"/>
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label" for="phone_number">رقم الهاتف</label>
                                        <input type="number" id="phone_number" name="phone_number" value="{{$teacher->phone_number}}" class="form-control" required />
                                        <x-form-err name="phone_number"/>
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label" for="education">المؤهل</label>
                                        <input type="text" id="education" name="education" class="form-control" value="{{$teacher->education}}" required />
                                        <x-form-err name="education"/>
                                    </div>
                                    <div class="col-6">
                                        <label class="form-label" for="address">محل الاقامه</label>
                                        <input id="address" name="address" class="form-control" value="{{$teacher->address}}"  required/>
                                ...
                                <div class="row g-3">
                                    <div class="col-md-6">
                                        ...
                                        <input type="date" id="hiring_date" value="{{$teacher->hiring_date}}"  name="hiring_date" class="form-control" required />
                                        <x-form-err name="hiring_date"/>
                                    </div>
                                    <div class="col-6">
                                        <label class="form-label" for="subject">المادة</label>
                                        <input id="subject" name="subject" class="form-control" value="{{$teacher->subject}}" required/>
                                        <x-form-err name="subject"/>
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label" for="duration">تحديد الفتره الدراسيه</label>
                                        <select id="duration" name="duration" class="form-select">
                                            <option {{$teacher->duration === 'صباحي'? 'selected': ''}} value="صباحي">صباحي</option>
                                            <option {{$teacher->duration === 'مسائي'? 'selected': ''}} value="مسائي">مسائي</option>
                                        </select>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <select id="academy_class_id" name="academy_class_id" class="form-select" required>
                                            @foreach($activeClasses as $class)
                                                <option {{$teacher->academy_class_id === $class->id? 'selected': ''}} value="{{ $class->id }}">{{ $class->name }}</option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="number" id="salary" name="salary" value="{{$teacher->salary}}"  class="form-control" required />
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <select id="is_archive" name="is_archive" class="form-select" >
                                            <option {{$teacher->is_archive? 'selected': ''}} value="1">مؤرشف</option>
                                            <option {{$teacher->is_archive? '': 'selected'}} value="0">مفعل</option>
                                        </select>
